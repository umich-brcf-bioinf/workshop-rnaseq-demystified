...
Based on our PCA results, we'll revise our DESeq object to add a covariate for "batch" and then repeat the filtering process. Then, we'll fit our model using the `DESeq` function and take a look at the objects we generate. This command applies the model to our data, using the sample information supplied when generating the initial `dds_batch` object so can take some time to run.
