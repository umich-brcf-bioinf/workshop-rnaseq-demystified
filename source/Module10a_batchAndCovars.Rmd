...
In this case, we weren't aware of any [covariates](https://methods-sagepub-com.proxy.lib.umich.edu/reference/encyc-of-research-design/n85.xml) that should be considered in our comparisons. However, let's look back at our initial PCA plot and see if there are any concerning patterns:
...
Before we proceed further, let's check the corresponding design matrix to ensure that our batches are not confounded, which would cause DESeq2 to fail to fit our model.
...
Just like in our initial analysis, now that we've initialized a `DESeq2` object that includes our batch labels, we can generate the `rlog` normalized count data and have that meta-data included:

```{r BatchNormalization}
rld_batch = rlog(dds_batch, blind = TRUE)
head(rld_batch)
```
...
```{r BatchPCA}
pca_plot_batch = plotPCA(rld_batch, intgroup = c('batch'), ntop = 500)
pca_plot_batch
```
...
To fit our updated model, just like for our intitial analysis, we use the `DESeq` function:

```{r FitModelCovariate}
dds_batch_fitted = DESeq(dds_batch)
resultsNames(dds_batch_fitted) 
```
Notice that we have two additional sets of results that include the `batch` labels. <What this means is that ...>

Let's look at these results before we compare between the two models.

```{r setBatchResults}
resultsBatch_deficient_vs_control = results(dds_batch_fitted, name = 'condition_deficient_vs_control')
head(resultsBatch_deficient_vs_control)

```

Before we proceed with generating summaries and visualizations like volcano plots, let's `save` our fitted DESeq R object for batch to file:
