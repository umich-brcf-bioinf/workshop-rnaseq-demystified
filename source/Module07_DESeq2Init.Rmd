...
samplesheet_batch = read.table("data/samplesheet.csv",
                       sep = ",",
                       header = TRUE,
                       row.names = 1)
```
...
```{r SamplesheetFactor}
# tidy version
samplesheet_ready <- samplesheet_batch %>% mutate(condition = factor(condition, 
...
```{r echo=FALSE, eval=FALSE}
# base version - add as dropdown option
samplesheet_ready <- samplesheet
samplesheet_ready$condition = factor(samplesheet_ready$condition,
                            levels = c('control', 'deficient'))
```

Notice that we set "control" as the first level as the factor. This ensures that the "Control" (or "Reference") group is the denominator in the default comparisons in our DESeq2 model.
...
For our dataset we only have one column we are interested in, that is `condition`. Although the design formula is required, it is not used at this step.
